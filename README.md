...
| Обход белых списков на Android | Краткая инструкция по настройке, подключению, отключению и проверке Android-клиента | [`android-whitelist-bypass/`](android-whitelist-bypass/) |
...
├── android-whitelist-bypass/   # инструкция по запуску обхода на Android
├── windows-whitelist-bypass/   # реализация обхода на Windows 11
├── methodology/                 # подробная методичка по белым спискам
...
| Белые списки | Аварийный VPN через FreeTurn/VK TURN + WireGuard | Рабочий; инструкции и реализации для Android и Windows 11 | Не затрагивать без отдельной задачи |
...
| Задача | Куда перейти |
|---|---|
| Подключить уже подготовленный Android-телефон | [`android-whitelist-bypass/`](android-whitelist-bypass/) |
| Установить или использовать fail-safe систему на Windows 11 | [`windows-whitelist-bypass/`](windows-whitelist-bypass/) |
| Понять архитектуру, ограничения и диагностику | [`methodology/`](methodology/) |

### Обход белых списков на Android

Для уже подготовленного телефона создана отдельная пользовательская инструкция [`android-whitelist-bypass/`](android-whitelist-bypass/). В ней собраны первоначальные требования, порядок подключения и отключения, прохождение CAPTCHA, проверка внешнего IP и быстрые действия при типовых ошибках.

Пошаговая инструкция: **[`android-whitelist-bypass/README.md`](android-whitelist-bypass/README.md)**.
